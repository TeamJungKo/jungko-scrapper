...
```bash
jungko-scrapper$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
jungko-scrapper$ sam local start-api
jungko-scrapper$ curl http://localhost:3000/
```
...
```bash
jungko-scrapper$ sam logs -n HelloWorldFunction --stack-name "jungko-scrapper" --tail
```
...
```bash
sam delete --stack-name "jungko-scrapper"
```
